...
(function() {
    ...
    const launchDelay = 5000;
    let launchIn;
    //Wait for page to load
    timeOutFunction = setTimeout(checkPageLoaded, checkPageLoadedDelay);

    function checkPageLoaded() {
        inpFriendsPlus = document.getElementById("friends+");
        inpEU = document.querySelector("img[alt=eu]");
        btnLaunch = document.querySelector("a.btn.btn-primary");
        VRCWebEls = [inpFriendsPlus, inpEU, btnLaunch];
        if (inpFriendsPlus == null || inpEU == null || btnLaunch == null) {
            countPageNotLoaded++;
